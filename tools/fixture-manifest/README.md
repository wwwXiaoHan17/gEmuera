...
版本化声明位于 `tools/fixture-manifest/manifest.json`。其中 `authorization.status=Verified` 只有在声明的许可证文件实际存在并被 hash 后才有效；`Unverified/MissingEvidence` 强制 `redistributionAllowed=false`。字节存在不代表授权可再分发。
