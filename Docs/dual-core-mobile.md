...
- `ENUMFUNC*` and `ENUMVAR*` enumerate the modern function/variable registries and write matches into either the supplied string variable array or `RESULTS`. `ENUMMACRO*` is intentionally not registered yet because the isolated modern core does not have a macro dictionary.
...
- `OUTPUTLOG` and `GETDOINGFUNCTION` are still intentionally deferred because they need the later Godot console/log and modern call-stack bridge instead of direct desktop runtime access.
